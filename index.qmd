...
My understanding of Houston's extreme rainfall became more nuanced as I went through the interative prior specification process. Houston natives (i.e., lab member), as well as members of my Climate and Water Lab, proved to be valuable knowledge sources for extreme value analysis in Houston.   
...
The observed data showed me that GEV parameters is not static and should be  changed to reflect your data. Additionally, I learned that to adequately constrain uncertainity and plot GEV well, domain data and empirical evidence in Bayesian analysis should compliment each other. An overreliance on domain knowledge, for example, could negatively skew the distribution of your data.
...
The same prior shouldn't be used for both stations, especially with stations that hav dissimilar climate conditions. Posterior differences incorporate observational data (geographic and climatic factors) with hypothesised priors, which enhances the ability to analyze regional hydroclimate variability.  
...
Compared to MLE approaches, Bayesian uncertainty quantification provides a more comprehensive way to model uncertainity. Model reliabilty increases with posterior approaches. 
...
I anticipate challenges with identifying the appropriate prior for the different rainfall datasets I will be working with, but this iterative workflow will help me better understand prior selection, and how to choose between Bayesian and MLE approaches. 
...
```{julia}
my_stnid = 779  # REQUIRED: replace with your chosen station ID (I chose Galveston)
...
#| output: false
return_level_priors = [
    ReturnLevelPrior(2, 5, 1.5),
    ReturnLevelPrior(10, 15, 4),
    ReturnLevelPrior(50, 13, 6),
    ReturnLevelPrior(100, 15, 8),
]
```
...
```{julia}
comparison_stnid = 600 # MODIFY THIS AND CHOOSE YOUR COMPARISON (Chose: 600/Rio Grande City)
